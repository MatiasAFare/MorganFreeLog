...
const PermisoModel = {
    ...
    create: (nombre, descripcion) => {
        try {
            const stmt = db.prepare('INSERT INTO permisos (nombre, descripcion) VALUES (?, ?)');
    ...
    update: (id, nombre, descripcion) => {
        try {
            const stmt = db.prepare('UPDATE permisos SET nombre = ?, descripcion = ? WHERE id = ?');
    ...
    assignToRole: (rolId, permisoId) => {
        const stmt = db.prepare('INSERT OR IGNORE INTO rol_permiso (rol_id, permiso_id) VALUES (?, ?)');
        const result = stmt.run(rolId, permisoId);
        return { changes: result.changes };
    },

    removeFromRole: (rolId, permisoId) => {
        const stmt = db.prepare('DELETE FROM rol_permiso WHERE rol_id = ? AND permiso_id = ?');
        const result = stmt.run(rolId, permisoId);
        return { changes: result.changes };
    },
