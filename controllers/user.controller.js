...
const userController = {
        ...
        try {
            ...
            const usersWithRoles = await Promise.all(users.map(async (user) => {
                if (user.rol_id) {
                    ...
                    return { ...user, rol_nombre: rol ? rol.nombre : 'Rol desconocido' };
                }
        ...
        try {
            ...
            if (!name || !email) {
                ...
                return res.render('users/new', {
                    ...
                    roles,
                    user: { name, email, rol_id }
                });
            }

            await UserService.createUser(name, email, rol_id || null);
            res.redirect('/users');
        } catch (error) {
            const roles = await RolService.getAll();
    ...
    handleDelete: async (req, res) => {
        try {
            ...
            if (result.changes === 0) {
                return res.redirect('/users?error=Usuario no encontrado');
            }
