...
const rolesController = {
  ...
  getRolById: async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: "Error fetching rol", error: error.message });
    }
  },
  ...
  createRol: async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: "Error creating rol", error: error.message });
    }
  },
  ...
  updateRol: async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: "Error updating rol", error: error.message });
    }
  },
  ...
  deleteRol: async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: "Error deleting rol", error: error.message });
    }
  },
  ...
  handleDelete: async (req, res) => {
    try {
      ...
      if (usuarios.length > 0) {
        return res.redirect(
          "/roles?error=No se puede eliminar el rol porque hay usuarios asignados a él"
        );
      }
